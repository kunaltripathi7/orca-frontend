...
const NavigationServerConfig = () => {
  const dispatch = useDispatch();
  return (
    // mt-auto for moving a flex to the bottom
    <section className="mt-auto flex flex-col items-center justify-center gap-2">
      <NavigationAction label="Explore communities" onClick={() => {}}>
        <Explore />
      </NavigationAction>
      <ActionTooltip side="right" align="center" label="Add a server">
        <button
          className="active:scale-9 group relative my-1 flex cursor-pointer items-center justify-center rounded-full shadow-md transition-all hover:scale-110 active:scale-90"
