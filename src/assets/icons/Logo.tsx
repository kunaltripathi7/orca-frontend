interface Props {
  className?: string;
}

const Logo = ({ className = "w-40 h-40" }: Props) => {
  return (
    <svg
      className={className}
      viewBox="0 0 40 40"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g clipPath="url(#clip0_528_5689)">
        <mask
          id="mask0_528_5689"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="2"
          y="1"
          width="36"
          height="38"
        >
          <path
            d="M16.0002 2.30941C18.4755 0.880346 21.525 0.880346 24.0002 2.30941L33.3208 7.69061C35.796 9.11967 37.3208 11.7607 37.3208 14.6188V25.3812C37.3208 28.2393 35.796 30.8803 33.3208 32.3094L24.0002 37.6906C21.525 39.1197 18.4755 39.1197 16.0002 37.6906L6.67974 32.3094C4.20453 30.8803 2.67974 28.2393 2.67974 25.3812V14.6188C2.67974 11.7607 4.20453 9.11967 6.67974 7.69061L16.0002 2.30941Z"
            fill="#C4C4C4"
          />
        </mask>
        <g mask="url(#mask0_528_5689)">
          <path
            d="M49 42H41.062C38.4102 42 34.4187 40.6052 31.215 38.5833C29.3988 39.7319 27.404 40.6238 25.2817 41.2079C23.912 41.72 22.4289 42 20.8805 42C20.6657 42 20.4522 41.9946 20.2401 41.984C19.9578 41.9946 19.6742 42 19.3893 42C14.5212 42 10.0198 40.4299 6.36388 37.7684C3.7801 36.7774 1.09516 35.8474 -0.832309 35.4357C-1.94589 35.1978 -3.03698 35.0693 -3.99033 34.957C-5.49633 34.7796 -6.65861 34.6427 -7.02283 34.1792C-7.56512 33.4892 -7.07602 32.0395 -6.78067 31.1641C-6.75208 31.0793 -6.7253 31 -6.70145 30.9272C-6.41706 30.0594 -6.62317 29.7027 -7.04762 29.2404C-7.47207 28.7782 -7.47483 28.2295 -7.03425 27.6996C-6.73925 27.3448 -6.08213 27.2543 -5.75209 27.2089C-5.68781 27.2 -5.63594 27.1929 -5.60156 27.1859C-5.72441 27.0893 -5.91031 26.9889 -6.11872 26.8764C-6.72519 26.549 -7.52222 26.1186 -7.51023 25.3803C-7.50556 25.0924 -7.18703 24.827 -6.80201 24.5061C-6.46748 24.2274 -6.08276 23.9068 -5.81008 23.4933C-5.34452 22.7873 -6.20745 22.3539 -6.83736 22.0376C-7.00102 21.9554 -7.14895 21.8811 -7.25377 21.812C-7.76201 21.4768 -7.99949 20.9696 -8 20.3172C-8.00063 19.5158 -7.34214 19.2583 -6.61092 18.9724C-6.27862 18.8424 -5.9313 18.7066 -5.62399 18.5112C-4.46549 17.7745 -3.28786 16.7861 -2.4362 16.0187C-2.16163 14.4417 -1.72031 12.9219 -1.13107 11.4783C2.01632 3.20917 10.065 -2.52252 19.2357 -2.52252C28.7872 -2.52252 37.7453 3.6721 40.6173 12.7816L49 42Z"
            fill="url(#paint0_angular_528_5689)"
          />
          <path
            d="M9.74997 23.5881C8.8316 25.3338 8.31189 27.3219 8.31189 29.4314C8.31189 36.3729 13.939 42 20.8805 42C27.8219 42 33.449 36.3729 33.449 29.4314C33.449 26.8869 32.6929 24.519 31.393 22.5401C30.158 27.2641 25.8611 30.75 20.75 30.75H9.74997V23.5881Z"
            fill="url(#paint1_angular_528_5689)"
          />
          <path
            d="M12.25 20.2945C14.5013 18.1672 17.5386 16.8629 20.8805 16.8629C24.0924 16.8629 27.0229 18.0677 29.2448 20.0501C29.0867 24.6055 25.344 28.25 20.75 28.25H12.25V20.2945Z"
            fill="url(#paint2_angular_528_5689)"
          />
          <path
            d="M43.9017 38.0956C45.1692 39.3641 42.5713 42 42.5713 42C37.911 42 29.1541 41.1687 26.2158 33.2435L25.1519 29.8339C29.0359 28.1361 31.75 24.26 31.75 19.75C31.75 13.6749 26.8252 8.75002 20.75 8.75002H12.2694C7.23445 7.54917 1.95941 9.6324 -0.887723 13.998C-1.29161 14.5685 -2.10546 14.2264 -1.92327 13.5516C0.606194 4.18279 9.31668 -2.52252 19.2357 -2.52252C27.6486 -2.52252 35.6011 2.28318 39.34 9.65354C40.321 11.5873 40.0495 13.8623 39.4007 15.9313C37.8682 20.8193 36.2758 30.4632 43.9017 38.0956Z"
            fill="url(#paint3_angular_528_5689)"
          />
          <path
            d="M24.4011 27.4281L21.1638 17.0541C20.4446 14.7494 19.0778 12.7462 17.2748 11.25H20.75C25.4445 11.25 29.25 15.0556 29.25 19.75C29.25 23.1376 27.2684 26.0623 24.4011 27.4281Z"
            fill="url(#paint4_angular_528_5689)"
          />
          <g style={{ mixBlendMode: "overlay" }}>
            <path
              d="M16 2.3094C18.4752 0.880338 21.5248 0.880338 24 2.3094L33.3205 7.6906C35.7957 9.11966 37.3205 11.7607 37.3205 14.6188V25.3812C37.3205 28.2393 35.7957 30.8803 33.3205 32.3094L24 37.6906C21.5248 39.1197 18.4752 39.1197 16 37.6906L6.67949 32.3094C4.20428 30.8803 2.67949 28.2393 2.67949 25.3812V14.6188C2.67949 11.7607 4.20428 9.11966 6.67949 7.6906L16 2.3094Z"
              fill="url(#paint5_linear_528_5689)"
              style={{ mixBlendMode: "overlay" }}
            />
            <path
              d="M16 2.3094C18.4752 0.880338 21.5248 0.880338 24 2.3094L33.3205 7.6906C35.7957 9.11966 37.3205 11.7607 37.3205 14.6188V25.3812C37.3205 28.2393 35.7957 30.8803 33.3205 32.3094L24 37.6906C21.5248 39.1197 18.4752 39.1197 16 37.6906L6.67949 32.3094C4.20428 30.8803 2.67949 28.2393 2.67949 25.3812V14.6188C2.67949 11.7607 4.20428 9.11966 6.67949 7.6906L16 2.3094Z"
              fill="url(#paint6_linear_528_5689)"
              fillOpacity="0.5"
            />
          </g>
        </g>
      </g>
      <defs>
        <radialGradient
          id="paint0_angular_528_5689"
          cx="0"
          cy="0"
          r="1"
          gradientUnits="userSpaceOnUse"
          gradientTransform="translate(21.6835 19.8136) rotate(35.2605) scale(29.0182 38.102)"
        >
          <stop offset="0.101333" stop-color="#612566" />
          <stop offset="0.12497" stop-color="#ED6A84" />
          <stop offset="0.396084" stop-color="#9E235F" />
          <stop offset="0.509199" stop-color="#612566" />
          <stop offset="0.58545" stop-color="#2C1842" />
          <stop offset="0.865758" stop-color="#21142C" />
        </radialGradient>
        <radialGradient
          id="paint1_angular_528_5689"
          cx="0"
          cy="0"
          r="1"
          gradientUnits="userSpaceOnUse"
          gradientTransform="translate(24.0982 21.4661) rotate(-31.3229) scale(21.2003 34.3807)"
        >
          <stop offset="0.101333" stop-color="#612566" />
          <stop offset="0.12497" stop-color="#ED6A84" />
          <stop offset="0.396084" stop-color="#9E235F" />
          <stop offset="0.509199" stop-color="#612566" />
          <stop offset="0.58545" stop-color="#2C1842" />
          <stop offset="0.865758" stop-color="#21142C" />
        </radialGradient>
        <radialGradient
          id="paint2_angular_528_5689"
          cx="0"
          cy="0"
          r="1"
          gradientUnits="userSpaceOnUse"
          gradientTransform="translate(24.0982 21.4661) rotate(-31.3229) scale(21.2003 34.3807)"
        >
          <stop offset="0.101333" stop-color="#612566" />
          <stop offset="0.12497" stop-color="#ED6A84" />
          <stop offset="0.396084" stop-color="#9E235F" />
          <stop offset="0.509199" stop-color="#612566" />
          <stop offset="0.58545" stop-color="#2C1842" />
          <stop offset="0.865758" stop-color="#21142C" />
        </radialGradient>
        <radialGradient
          id="paint3_angular_528_5689"
          cx="0"
          cy="0"
          r="1"
          gradientUnits="userSpaceOnUse"
          gradientTransform="translate(23.1995 33.5473) rotate(54.3486) scale(19.0784 10.2935)"
        >
          <stop offset="0.000195603" stop-color="#2C1842" />
          <stop offset="0.0990559" stop-color="#21142C" />
          <stop offset="0.220158" stop-color="#080808" />
          <stop offset="0.381095" stop-color="#ED6A84" />
          <stop offset="0.589726" stop-color="#9E235F" />
          <stop offset="0.6817" stop-color="#612566" />
        </radialGradient>
        <radialGradient
          id="paint4_angular_528_5689"
          cx="0"
          cy="0"
          r="1"
          gradientUnits="userSpaceOnUse"
          gradientTransform="translate(23.1995 33.5473) rotate(54.3486) scale(19.0784 10.2935)"
        >
          <stop offset="0.000195603" stop-color="#2C1842" />
          <stop offset="0.0990559" stop-color="#21142C" />
          <stop offset="0.220158" stop-color="#080808" />
          <stop offset="0.381095" stop-color="#ED6A84" />
          <stop offset="0.589726" stop-color="#9E235F" />
          <stop offset="0.6817" stop-color="#612566" />
        </radialGradient>
        <linearGradient
          id="paint5_linear_528_5689"
          x1="36.5"
          y1="8"
          x2="18.0645"
          y2="45.249"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#C441F4" />
          <stop offset="1" stop-color="#00DAEA" />
        </linearGradient>
        <linearGradient
          id="paint6_linear_528_5689"
          x1="20"
          y1="0"
          x2="20"
          y2="40"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="white" />
          <stop offset="1" stopColor="#2596FF" stopOpacity="0.79" />
        </linearGradient>
        <clipPath id="clip0_528_5689">
          <rect width="40" height="40" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
};

export default Logo;
